...
11. `Service.startup(...)` 默认是 module-first。主推 `source=已 import 的 module`，也可以传 `entry_module="..."`。startup 不做远程代码 upload，也不把业务 module 重新打包成远端 artifact；本地 executor worker 按模块名直接 import 本地文件，`worker_count` 对应真实进程数。如果要验证 upload/artifact/资源文件/依赖处理，请用显式 artifact 参数。运行配置建议通过 `initial_globals={...}` 或后续 `update_globals(...)` 注入，不推荐用 `cwd` / `os.environ` 这类进程全局状态承载单个服务配置。
    `worker_backend="process"` 是默认值；`worker_backend="inline"` 仅供需要与 owner 共享内存状态的控制型服务使用。inline 的 HTTP/IPC 接入可以并发，但每个 service 的实际执行并发由独立 semaphore 严格限制为 `worker_count`；CPU 密集型业务服务仍不应使用。
12. 空 `target` 不表示通用 local 模式。`Service.deploy(...)`、`Service.connect(...)`、`TaskPool.open(...)` 仍然必须显式传入 `target`
13. `Service.startup(...).foo.sync(...)` 在 startup 的非 local 模式和 local 模式下都是当前 startup node 对自己服务的本地调用门面：本进程内 proxy 直接调用 `StartupServiceNode.call_service(...)`，随后进入 executor host 和本地进程 worker，不经过 Discovery、Gateway 或 service HTTP；它只是调用便利，不表示 startup service 加入动态服务组
14. `target="local"` 是显式本地 IPC 模式：`Service.startup(target="local", ...)` 和 `Service.deploy(target="local", source=module)` 都会返回本地 owner handle、按 `service_name` 写入本机 IPC registry，并使用 `source_kind="module_import"` + 多进程 executor；worker 直接从本机 import root 加载模块，不生成业务代码 bridge。同名 local service 已存活时启动会失败，`Service.connect(target="local", service_name=...)` 通过该 registry 连接到对应本地服务。显式 artifact/deps/resource/package 参数继续走完整 artifact 路径。
...
18. `TaskPool.open(target="local", ...)` 创建 opener 私有的本地 task pool，任务提交到当前进程持有的 `NodeControlState`，再由 `ExecutorHost + ProcessPoolExecutor` 执行；module、module-level callable 和显式 artifact 使用同一套 runtime，不再存在独立的线程 TaskPool。`unordered(...)`、`aunordered(...)`、`iter_items(...)` 等高层 wrapper 复用同一套 session 逻辑；TaskPool 没有 connect 语义，其他进程不能接入这个 pool
