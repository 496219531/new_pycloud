...
11. `Service.startup(...)` 默认是 module-first。主推 `source=已 import 的 module`，也可以传 `entry_module="..."` 由本进程 import 后直接挂载。startup 是本机快启动语义，不做远程代码 upload，也不把 module 重新打包成远端 artifact；如果要验证 upload/artifact/资源文件/依赖处理，请用 `Service.deploy(target="local", ...)`。运行配置建议通过 `initial_globals={...}` 或后续 `update_globals(...)` 注入，不推荐用 `cwd` / `os.environ` 这类进程全局状态承载单个服务配置。
