...
node = Service.startup(
    ...
    source=calc_service,
    bind="0.0.0.0:18080",
    initial_globals={"CALC_DATA_DIR": "/srv/calc/data"},
)
...
`Service.startup(...)` 主推传入已 import 的 module：`source=calc_service`。startup 是本机部署语义，不需要远程 upload code；显式 `entry_module`、bytes/path artifact 或 `package_format` 仍作为兼容路径保留。启动前必须可见的运行配置建议通过 `initial_globals={...}` 注入，后续热更新再用 owner handle 的 `update_globals(...)`；不要用 `cwd` / `os.environ` 这类进程全局状态影响同进程内其它服务。
