...
"""Service-session cache helpers and service execution session extracted from client."""
...
ServiceExecutionSession = client_mod.ServiceExecutionSession
OBJECT_CHUNK_SIZE_BYTES = client_mod.OBJECT_CHUNK_SIZE_BYTES
_DEFAULT_EXPORT_DECORATOR = client_mod._DEFAULT_EXPORT_DECORATOR
matches_python_runtime = client_mod.matches_python_runtime
normalize_python_runtime_spec = client_mod.normalize_python_runtime_spec
_SERVICE_SESSION_SCHEMA_VERSION = client_mod._SERVICE_SESSION_SCHEMA_VERSION
_SERVICE_SESSION_LOCK_GUARD = client_mod._SERVICE_SESSION_LOCK_GUARD
_SERVICE_SESSION_LOCKED_PATHS = client_mod._SERVICE_SESSION_LOCKED_PATHS
_default_service_session_cache_dir = client_mod._default_service_session_cache_dir
_sanitize_session_cache_part = client_mod._sanitize_session_cache_part
_ensure_private_dir = client_mod._ensure_private_dir
...
ServiceSessionClient = _forward("ServiceSessionClient")
Artifact = _forward("Artifact")
ArtifactDeps = _forward("ArtifactDeps")
_coerce_artifact_deps = _forward("_coerce_artifact_deps")
_prepare_code_blob = _forward("_prepare_code_blob")
_normalize_artifact_input = _forward("_normalize_artifact_input")
_prepare_artifact = _forward("_prepare_artifact")
_source_module_from_entry_module_arg = _forward("_source_module_from_entry_module_arg")
_source_func_from_entry_callable_arg = _forward("_source_func_from_entry_callable_arg")
_normalize_entry_module_arg = _forward("_normalize_entry_module_arg")
_normalize_entry_callable_arg = _forward("_normalize_entry_callable_arg")
_default_entry_module_for_func = _forward("_default_entry_module_for_func")
_default_entry_module_for_module = _forward("_default_entry_module_for_module")
_resolve_package_format = _forward("_resolve_package_format")
_default_artifact_filename = _forward("_default_artifact_filename")
_get_local_ip = _forward("_get_local_ip")
_infer_entry_module_from_artifact_path = _forward("_infer_entry_module_from_artifact_path")
_artifact_code_version = _forward("_artifact_code_version")
_emit_owner_notice = _forward("_emit_owner_notice")
_retry_infocenter_request = _forward("_retry_infocenter_request")
_filter_nodes_by_runtime = _forward("_filter_nodes_by_runtime")
_summarize_discovered_nodes = _forward("_summarize_discovered_nodes")
_timestamp_to_datetime = _forward("_timestamp_to_datetime")
_write_private_json = _forward("_write_private_json")
_put_data_via_clients = _forward("_put_data_via_clients")
_prepare_managed_globals_values_for_upload = _forward("_prepare_managed_globals_values_for_upload")
_resolve_high_level_service_data = _forward("_resolve_high_level_service_data")
_resolve_high_level_service_results = _forward("_resolve_high_level_service_results")
_serialize_arrow_compatible = _forward("_serialize_arrow_compatible")
@dataclass
class _ServiceSessionFileLock:
    ...
    def acquire(self) -> "_ServiceSessionFileLock":
        ...
        with client_mod._SERVICE_SESSION_LOCK_GUARD:
            if normalized in client_mod._SERVICE_SESSION_LOCKED_PATHS:
                raise RuntimeError(f"local deploy session already holds cache lock: {self.path}")
            client_mod._SERVICE_SESSION_LOCKED_PATHS.add(normalized)
        try:
            client_mod._ensure_private_dir(self.path.parent)
            fp = open(self.path, "a+b")
        except Exception:
            with client_mod._SERVICE_SESSION_LOCK_GUARD:
                client_mod._SERVICE_SESSION_LOCKED_PATHS.discard(normalized)
            raise
        try:
            try:
...
def _service_session_cache_file(
    *,
    ...
    base_dir = Path(cache_dir).expanduser() if str(cache_dir).strip() else client_mod._default_service_session_cache_dir()
    return (
        base_dir
        / client_mod._sanitize_session_cache_part(owner_client_id)
        / f"{client_mod._sanitize_session_cache_part(service_name)}.json"
    )
...
def _load_service_session_cache(
    *,
    ...
    if not isinstance(payload, dict):
        return None
    if int(payload.get("schema_version", 0) or 0) != client_mod._SERVICE_SESSION_SCHEMA_VERSION:
        return None
...
@dataclass
class ServiceGroup(ServiceExecutionSession):
