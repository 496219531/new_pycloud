...
class InfoCenterClient:
    ...
    def list_selected_nodes(
        ...
        *,
        ...
        normalized_node_ids = [str(value).strip() for value in (node_ids or []) if str(value).strip()]
        normalized_instance_ids = [str(value).strip() for value in (node_instance_ids or []) if str(value).strip()]
        if not normalized_node_ids and not normalized_instance_ids:
        ...
        params = "&".join(
            [
                ...
                f"tags={','.join([x for x in (tags or []) if x])}",
                f"limit={max(1, int(limit))}",
                f"node_ids={','.join(normalized_node_ids)}",
                f"node_instance_ids={','.join(normalized_instance_ids)}",
            ]
        )
        resp = http_json_request(
            base_url=self.base_url,
        ...
        *,
        ...
        nodes = list(
            self.list_selected_nodes(
                ...
                tags=tags,
                limit=limit,
                node_ids=requested_node_ids if requested_node_ids else None,
                node_instance_ids=requested_instance_ids if requested_instance_ids else None,
            )
        )
