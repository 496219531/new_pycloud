...
def _parse_services(payload: object) -> Dict[str, NodeServiceState]:
    ...
    for item in payload or []:
        ...
        out[service_id] = NodeServiceState(
            ...
            requested_workers=max(0, int(item.get("requested_workers", item.get("worker_count", 0)) or 0)),
            alive_workers=max(0, int(item.get("alive_workers", 0) or 0)),
            busy_workers=max(0, int(item.get("busy_workers", 0) or 0)),
            queued=max(0, int(item.get("queued", 0) or 0)),
            worker_pids=tuple(sorted(int(pid) for pid in item.get("worker_pids", ()) if int(pid) > 0)),
            executor_generation=max(0, int(item.get("executor_generation", 0) or 0)),
            ...
            created_at=_parse_optional_dt(item.get("created_at") or item.get("created_at_ts")),
            ...
            signal_cursor=max(0, int(item.get("signal_cursor", 0) or 0)),
        )
    return out
        ...
        merged.append(
            {
                ...
                "readiness": str(getattr(primary, "readiness", "") or ""),
                "readiness_reason": "; ".join(
                    sorted(
                        {
                            str(getattr(item, "readiness_reason", "") or "").strip()
                            for item in ordered
                            if str(getattr(item, "readiness_reason", "") or "").strip()
                        }
                    )
                ),
...
def _serialize_service(service: NodeServiceState, *, node_healthy: bool = True) -> Dict[str, object]:
    ...
    return {
        ...
        "operation_updated_at": (
            _dt_text(getattr(service, "operation_updated_at", None))
            if getattr(service, "operation_updated_at", None) is not None
            else ""
        ),
        ...
        "task_pools": [
            {
                ...
                "operation_updated_at": (
                    _dt_text(getattr(pool, "operation_updated_at", None))
            ...
            node_payload["services"].append(
                {
                    ...
                    "operation_updated_at": (
                        _dt_text(getattr(svc, "operation_updated_at", ""))
                        if getattr(svc, "operation_updated_at", None) is not None
                        else ""
                    ),
            ...
            node_payload["task_pools"].append(
                {
                    ...
                    "operation_id": str(getattr(pool, "operation_id", "") or ""),
                    "operation_updated_at": (
                        _dt_text(getattr(pool, "operation_updated_at", ""))
                        if getattr(pool, "operation_updated_at", None) is not None
                        else ""
                    ),
                    "degraded": bool(getattr(pool, "degraded", False)),
...
def _render_ops_page(state: InfoCenterState, job_queue: Optional[JobQueueManager] = None, *, _snapshot_only: bool = False):
    ...
    for node in nodes:
        ...
        for pool in task_pools:
            ...
            pool_entries.append((
                ...
                f"<td>{_ops_status_badge(str(getattr(pool, 'readiness', '') or '-'))}</td>"
                f"<td>{html.escape(str(getattr(pool, 'create_stage', '') or '-'))}</td>"
    ...
    service_headers = [
        ...
        "deployed_at", "lease_expire_at", "readiness_reason", "dependency_modules", "failure_reason", "http_base_url",
    ]
    pool_headers = [
        "node_id", "instance_id", "pool_name", "pool_id", "owner_client_id", "status", "resource", "readiness", "stage", "workers", "alive", "tasks", "in_flight",
        "calls", "errors", "avg_total_ms", "avg_child_decode_ms", "avg_child_invoke_ms", "avg_child_encode_ms",
        "last_executor_create_ms", "avg_warmup_ms", "executor_rebuild_count", "code_version", "deployed_at",
    ...
    return (
        ...
        "body:not(.show-details) .ops-table--nodes :is(th,td):nth-child(2),body:not(.show-details) .ops-table--nodes :is(th,td):nth-child(3),body:not(.show-details) .ops-table--nodes :is(th,td):nth-child(8),body:not(.show-details) .ops-table--nodes :is(th,td):nth-child(11),body:not(.show-details) .ops-table--nodes :is(th,td):nth-child(n+13):nth-child(-n+29),body:not(.show-details) .ops-table--nodes :is(th,td):nth-child(31){display:none;}"
        ...
        "body:not(.show-details) .ops-table--services :is(th,td):nth-child(2),body:not(.show-details) .ops-table--services :is(th,td):nth-child(4),body:not(.show-details) .ops-table--services :is(th,td):nth-child(n+16):nth-child(-n+18),body:not(.show-details) .ops-table--services :is(th,td):nth-child(20),body:not(.show-details) .ops-table--services :is(th,td):nth-child(24){display:none;}"
        "body:not(.show-details) .ops-table--pools :is(th,td):nth-child(2),body:not(.show-details) .ops-table--pools :is(th,td):nth-child(4),body:not(.show-details) .ops-table--pools :is(th,td):nth-child(5),body:not(.show-details) .ops-table--pools :is(th,td):nth-child(n+17):nth-child(-n+23),body:not(.show-details) .ops-table--pools :is(th,td):nth-child(n+25):nth-child(-n+27){display:none;}"
                ...
                if parsed.path == "/services/routes":
                    ...
                    for item in routes:
                        ...
                        row["operation_updated_at"] = (
                            _dt_text(item.get("operation_updated_at"))
                            if item.get("operation_updated_at") is not None
                            else ""
                        )
                        row["failure_at"] = (
