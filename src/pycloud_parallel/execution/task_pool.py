...
def _build_local_task_pool(
    ...
    *,
    ...
    if callable(source) and not inspect.ismodule(source):
        callable_name = str(getattr(source, "__name__", "") or "").strip()
        qualified_name = str(getattr(source, "__qualname__", callable_name) or callable_name).strip()
        module_name = str(getattr(source, "__module__", "") or "").strip()
        if not callable_name or callable_name == "<lambda>" or "<locals>" in qualified_name or not module_name:
            raise ValueError(
                "local TaskPool process workers require an importable module-level callable; "
                "move the callable to a module or pass source=module"
            )
